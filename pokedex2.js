...
function addPokemon(pokemon){
    $('#pokedex').append('<div class="card col-xs-12 col-sm-3 m-3 border-5 border border-dark">' + 
                            '<img src="'+image+'" class="card-img-top" alt="...">' + 
                            '<div class="card-body">' + '<hr>' +
                                '<h5 class="card-title text-center">' + pokemon.name + '</h5>' +
                                '<button class="btn btn-warning btn-pok " data-bs-toggle="modal" data-bs-target="#pokemon-data" data-pokemon='+ pokemon.name +'> Quiero saber más de este pokemon' + '</button>'
                            + '</div>' 
                        + '</div>')
}
    ...
    fetch(url_pok + pokemon)
        ...
        .then(function(data){
            ...
            data.types.forEach(function(tipo){
                ...
                $('#pokemon-types').append('<button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#damage-data" data-type='+ tipo.type.name +'> Ver relaciones de daño' + '</button>')
            ...
            data.abilities.forEach(function(habilidad){
                ...
                $('#pokemon-abilities').append('<button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#others-pokemon-ability" data-ability='+ habilidad.ability.name +'> Otros pokemoens con esta habilidad' + '</button>')
            })
